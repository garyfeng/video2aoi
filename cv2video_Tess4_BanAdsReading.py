...
def processVideo(v):
    ...
    tabFileList=glob.glob("r_tab1_*.png")
        ...
        if flag:
            ...
            if (frameEngine.frameChanged(halfFrame) or forcedCalc):
                ...
                if taskName is None:
                    ...
                    logging.info("STATUS:\ttaskname='"+str(taskName)+"'\tessay='"+str(lastEssayID)+"'\ttab1='"+str(tab1name)+"'\ttab2='"+str(tab2name)+"'\t"+txt)
                elif taskName=="desktop":
                    ...
                    for (f, sig) in zip(tabFileList, tabImageList):
                        ...
                        print "testing tab1 = "+f
                        if res is not None:
                            # found match, break
                            taskSigLoc, minVal=res
                            # get task name
                            ptn=re.compile("r_tab1_(.+)\.png")  # can be moved outside of loop
                            dump= ptn.match(f)
                            if dump is not None: tab1name = dump.group(1)
                            logging.info("Tab1="+str(tab1name)+"\tLocation="+str(taskSigLoc)+"\tminVal="+str(minVal))
                            print "Tab1="+str(tab1name)+"\tLocation="+str(taskSigLoc)+"\tminVal="+str(minVal)
                            # break at the first found signature
                            break 
                    if (tab1name is None): 
                        logging.info("Tab= None\tLocation="+str(taskSigLoc)+"\tminVal="+str(minVal))
                        print "Tab= None\tLocation="+str(taskSigLoc)+"\tminVal="+str(minVal)
                    ...
                    # do OCR for tab1
                    if (tab1name is not None and lastEssayID != "falseID"):
                        tess.image2txt(cv2.cvtColor(np.copy(frame[tab1[0][1]:tab1[1][1], tab1[0][0]:tab1[1][0]]), cv.CV_RGB2GRAY))
                        html=tess.getHtml()
                        logging.info("AOI BEGIN:\tTab1='"+str(tab1name)+"'\tlocation=" + str(tab1)+"\t"+txt)
                        logging.info(html)
                        logging.info("AOI END:\tTab1='"+str(tab1name)+"'\tlocation=" + str(tab1)+"\t"+txt)
                    # do OCR for tab2
                    if (tab2name is not None and lastEssayID != "falseID"):
                        tess.image2txt(cv2.cvtColor(np.copy(frame[tab2[0][1]:tab2[1][1], tab2[0][0]:tab2[1][0]]), cv.CV_RGB2GRAY))
                        html=tess.getHtml()
                        logging.info("AOI BEGIN:\tTab2='"+str(tab2name)+"'\tlocation=" + str(tab2)+"\t"+txt)
                        logging.info(html)
                        logging.info("AOI END:\tTab2='"+str(tab2name)+"'\tlocation=" + str(tab2)+"\t"+txt)
                    # do OCR for the whole area, just in case
                    if (lastEssayID != "falseID"):
                        tess.image2txt(cv2.cvtColor(np.copy(frame[tab[0][1]:tab[1][1], tab[0][0]:tab[1][0]]), cv.CV_RGB2GRAY))
                        html=tess.getHtml()
                        logging.info("AOI BEGIN:\tTab1='wholepage'\tlocation=" + str(tab)+"\t"+txt)
                        logging.info(html)
                        logging.info("AOI END:\tTab1='wholepage'\tlocation=" + str(tab)+"\t"+txt)
    ...
    # OCR scrollImage
    if (tab1name is not None and tab2name is not None):
        #output
        logging.info("STATUS:\ttaskname='"+str(taskName)+"'\tessay='"+str(lastEssayID)+"'\ttab1='"+str(tab1name)+"'\ttab2='"+str(tab2name)+"'\t"+txt)
        # left tab
        tess.image2txt(cv2.cvtColor(np.copy(frame[307:765, 342:829]), cv.CV_RGB2GRAY))
        html=tess.getHtml()
        logging.info("AOI BEGIN:\tTab1='"+str(tab1name)+"'\tlocation=" + str(textarea)+"\t"+txt)
        logging.info(html)
        logging.info("AOI END:\tTab1='"+str(tab1name)+"'\tlocation=" + str(textarea)+"\t"+txt)
        # right tab
...
# this is the whole right tab
textarea=((849, 280), (849+495, 280+555))
logging.info("Text Area @ location " + str(textarea))


tab1=((336, 280), (336+495, 280+555))
tab2=((849, 280), (849+495, 280+555))
tab =((336, 280), (336+1010,280+555))
logging.info("Tab1 @ location " + str(tab1))
logging.info("Tab2 @ location " + str(tab2))
logging.info("Tab  @ location " + str(tab))
