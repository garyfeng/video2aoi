...
def draw_match(img1, img2, p1, p2, status = None, H = None):
    ...
    if H is not None:
        ...
        vis = np.zeros((vish, visw), np.uint8)
        px=min([x for x,y in corners])
        py=min([y for x,y in corners])
        print x1, y1, x2, y2, visw, vish, px,py
        vis[py-y1:py+h1-y1, px-x1:px+w1-x1] = img1
        vis[0-y1:h2-y1, 0-x1:w2-x1] = img2
        vis = cv2.cvtColor(vis, cv2.COLOR_GRAY2BGR)
...
def draw_match_old (img1, img2, p1, p2, status = None, H = None):
    h1, w1 = img1.shape[:2]
    h2, w2 = img2.shape[:2]
    vis = np.zeros((max(h1, h2), w1+w2), np.uint8)
    vis[:h1, :w1] = img1
