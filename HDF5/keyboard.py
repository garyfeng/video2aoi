...
def TypeString(string_to_type,
    ...
    for char in string_to_type:
        ...
        # Gary Feng, 2012-7-25
        # If this is \E, type ESC.
        if next_escaped and char == 'E':
            # This just wraps a pair of PressKey calls with an intervening
            # delay
            PressKey(True, win32con.VK_ESCAPE)
            time.sleep(keystroke_time)
            PressKey(False, win32con.VK_ESCAPE)
            next_escaped = False
            handled = True
        ...
        # @@@ Gary Feng: Need to handle double escapes for
        if next_escaped and char == "\\":
            next_escaped = False
            handled = False
        ...
        if not handled:
            ...
            # Type the key with the specified length, then wait the specified
            # delay
...
def TypeLiteralString(string_to_type,
                      use_modifiers=False,
                      keystroke_time=0,
                      time_between_keystrokes=0):
    """Simulate typing a string on the keyboard.
    ...
    Args:
      string_to_type: the string to print
      use_modifiers: ALWAYS FALSE; NO MODIFICATIONS. Just ASCII keys
      keystroke_time: length of time (in secondes) to "hold down" the key
      time_between_keystrokes: length of time (seconds) to pause between keys
