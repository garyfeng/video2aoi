...
def logEvents (allevents, aoilist, lastVTime, vTime, tOffset=0):
    ...
    for e in frameEvents:
        ...
        if len(aoilist)==0:
            ...
            activeAOI=aoilist[aoilist.x1<=x ]
            activeAOI=activeAOI[activeAOI.x2>x]
            activeAOI=activeAOI[activeAOI.y1<=y]
            activeAOI=activeAOI[activeAOI.y2>y]
            if len(activeAOI)>0:
...
def p2Task(k, value, context):
    ...
    if "match" in value:
        ...
        if threshold == -99:
            ...
            res = frameEngine.findMatch(img, sig, -99)
        else:
...
def processVideo(v):
    ...
    if processGazeLog:
        ...
        gaze = alldata[alldata.event=="gaze"]
    ...
    while video.grab():
        ...
        if flag:    
            ...
            ################################################
            #frameChanged = frameEngine.frameChanged(frame, 99) #<-- numba doesn't like optional parameters
            frame = np.array(frame)
            frameChanged = frameEngine.frameChanged(frame, 99)
...
def main_test():
    ''' a test function to work with line_profiler: fixed video file, parameters, etc. '''
    global yamlconfig, tess, parser, frameEngine, startFrame, showVideo, jumpAhead, txt, toffset, skimmingMode, logLevel
    ...
    yamlfile = "line_profiler_test.yaml"
    yamlconfig = yaml.load(open(yamlfile))
    assert "tasks" in yamlconfig
    assert "study" in yamlconfig
